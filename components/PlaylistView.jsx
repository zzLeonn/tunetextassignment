...
const PlaylistView = ({ globalPlaylistId, setGlobalCurrentSongId, setGlobalIsTrackPlaying, setView, setGlobalArtistId }) => {
    ...
    const [selectedTrackId, setSelectedTrackId] = useState(null); // Track the selected song ID
                ...
                <div className='text-white px-8 flex flex-col space-y-1 pb-28'>
                    {playlistData?.tracks.items.map((track, i) => (
                        <Song
                            ...
                            sno={i}
                            track={track.track}
                            isSelected={selectedTrackId === track.track.id} // Pass whether this song is selected
                            onSelect={() => setSelectedTrackId(track.track.id)} // Update selected song ID
